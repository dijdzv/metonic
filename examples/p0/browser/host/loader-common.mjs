const REQUIRED = ['init', 'resize', 'move_to', 'activate', 'field', 'scene_pointer', 'scene_key', 'task_begin', 'task_complete', 'task_fail', 'task_cancel', 'task_dispose', 'task_field', 'font_begin', 'font_put', 'font_commit', 'text_begin', 'text_put', 'text_commit', 'editor_render', 'view_layer_count', 'view_layer_field', 'view_layer_pixel', 'view_field', 'view_label_unit', 'view_focus', 'text_width', 'text_height', 'text_pixel', 'text_dispose'];

import { wasmImportObject } from './webapi.mjs';
import { createImports } from './websys-input.mjs';

export function wasmImports() {
  let buffer = '';
  const websys = createImports();
  websys.websys.input_document = () => document;
  websys.websys.input_window = () => window;
  return { ...websys, ...wasmImportObject, websys: websys.websys, console: { log: (...values) => console.log(...values) }, spectest: { print_char(codepoint) {
    buffer += String.fromCodePoint(Number(codepoint));
    if (buffer.includes('\n') || buffer.length >= 1024) {
      console.warn(buffer);
      buffer = '';
    }
  } } };
}

export function validateExports(exports, target) {
  for (const name of ['scene_gpu_init', 'scene_gpu_record', 'scene_gpu_dispose', 'view_place']) {
    if (typeof exports[name] !== 'function') throw new Error(`${target} artifact is missing required export: ${name}`);
  }
  if (target === 'js' && typeof exports.font_receive !== 'function') {
    throw new Error('js artifact is missing required export: font_receive');
  }
  for (const name of [...REQUIRED, 'text_gpu_init', 'text_gpu_add', 'text_gpu_commit', 'text_gpu_abort', 'text_gpu_record', 'text_gpu_dispose', 'font_fetch', 'font_fetch_cancel', 'view_layer_bytes', 'inputs_start', 'inputs_stop', 'inputs_error', 'inputs_composing', 'http_start', 'http_cancel', 'schedule_task', 'scheduled_tasks_reset', 'scheduled_task_count', 'scheduled_task_rejections']) {
    if (typeof exports[name] !== 'function') {
      throw new Error(`${target} artifact is missing required export: ${name}`);
    }
  }
  return exports;
}
