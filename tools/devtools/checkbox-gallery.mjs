...
export async function verifyCheckboxGallery(browser, baseUrl, outputDir) {
  ...
  for (const target of ['js', 'wasm-gc']) {
    ...
    try {
      ...
      await page.getByRole('button', { name: 'Rotate rows', exact: true }).click();
      assert.equal(await option('Item 1').getAttribute('aria-selected'), 'true');
      assert.equal(await page.getByRole('option').first().getAttribute('aria-label'), 'Item 1');
      assert.equal(await option('Item 4').getAttribute('aria-disabled'), 'true');
      const single = page.getByRole('textbox', { name: 'Single line' });
      const multi = page.getByRole('textbox', { name: 'Multiple lines' });
      assert.equal(await page.getByRole('textbox', { name: 'Disabled input' }).isDisabled(), true);
      assert.equal(await page.getByRole('button', { name: 'Disabled button' }).isDisabled(), true);
      await single.fill('updated key');
      await multi.fill('one\ntwo');
      assert.equal(await single.inputValue(), 'updated key');
      assert.equal(await multi.inputValue(), 'one\ntwo');
      ...
      await page.waitForFunction(() => document.querySelector('#controls [role="status"]')?.textContent === 'Quote: 112');
      assert.equal(await status.getAttribute('aria-live'), 'polite');
      const themedBefore = await canvas.screenshot();
      await page.getByRole('button', { name: 'Switch theme', exact: true }).click();
      await page.evaluate(() => new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve))));
      assert.notDeepEqual(await canvas.screenshot(), themedBefore, 'Theme event did not repaint');
