...
function factory() {
  ...
  server.registerTool('native_snapshot', {
    ...
    try { return result(await session.client.request('snapshot', {}, { signal: ctx.mcpReq.signal })); } catch (error) { return failure(error); }
  });
  server.registerTool('native_move', {
    ...
    try { return result(await session.client.request('move', { x, y, expected_revision }, { signal: ctx.mcpReq.signal })); } catch (error) { return failure(error); }
  });
  server.registerTool('native_resize', {
    ...
    try { return result(await session.client.request('resize', { width, height, expected_revision }, { signal: ctx.mcpReq.signal })); } catch (error) { return failure(error); }
  });
  server.registerTool('native_activate', {
    ...
    try { return result(await session.client.request('activate', { expected_revision }, { signal: ctx.mcpReq.signal })); } catch (error) { return failure(error); }
  });
  server.registerTool('native_capture', {
    ...
    try {
      ...
      const rgba = captured.rgba;
      const png = await encodeCapture({ response, rgba });
      return result(response, { content: [{ type: 'text', text: JSON.stringify({ session_id: sessionId, ...response }) }, { type: 'image', data: png.toString('base64'), mimeType: 'image/png' }] });
