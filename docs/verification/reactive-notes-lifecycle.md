...
| Browser host, JS and WasmGC | `mise -C consumers/metonic-notes run verify:browser` | Collection, retained rows, search, autosave, restart and CLI/MCP control scenarios passed on both targets |
| Windows native host | `mise -C consumers/metonic-notes run verify:native` | Native Notes operation probe and window cleanup passed |
