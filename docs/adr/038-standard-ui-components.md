...
The common selectable list has `List` and `ListItem` identities. Its browser accessibility mapping is `listbox` and `option`: WAI-ARIA `listitem` is structural and does not support `aria-selected`, while `option` does. Native AccessKit retains `List` and `ListItem` and exposes selection through the platform selection pattern. The host mappings preserve one common selection and navigation contract rather than imposing a misleading ARIA state on a structural list item.

The first List API accepts an application-owned `Signal[Key?]` for single selection. Activation writes the selected key; focus navigation alone does not change selection. Reordering retains each row owner and selection by key. Removing the selected row clears the Signal synchronously, while removing the focused row moves focus to the next enabled row, then the previous enabled row if no next row remains. Recreating a key receives a fresh semantic generation, so a late event for the removed row remains stale. The browser keeps the option controls as siblings of the listbox overlay and relates them with `aria-owns`; the overlay does not intercept pointer input. A general `Controlled[Key?]` callback would need a separate contract for synchronously invalidating selection during row removal, so it is not used for this first List API.

Status presentation is a stable, non-focusable semantic node. The application derives `Waiting`, `Loading`, `RetainedPrevious`, `Rejected`, `Failed`, `Canceled`, or `Ready` from its existing `Current`/publication result; the component adds no async state machine. `RetainedPrevious` always includes the word “previous” and never has the success state of a current result. Browser status text is a polite, atomic live region updated only when the message changes; native AccessKit exposes a Status node. Messages longer than 160 bytes fall back to a generic phase message. The framework does not copy raw failure causes or private note/input text into status announcements. A retry action remains a separate Button on the same owner/event path.
