...
| --- | --- | --- |
| Time | `effects/clock.Clock`; `async_runtime/clock.SystemClock::new()` for production; `VirtualClock` for tests | Select a clock when work needs one. One production instance supports repeated and concurrent waits; cancellation belongs to each async task, not the clock. |
